...
class GalleryPage extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    final displayImages = cubit.selectedGalleryPaths.isNotEmpty
          ...
          Expanded(
            child: displayImages.isEmpty
                ...
                : GridView.builder(
                    ...
                    itemBuilder: (_, index) => _buildImageItem(index),
                  ),
          ),
          const SizedBox(height: 16),
          _buildBottomActions(isLoading, displayImages.length),
        ],
      ),
    );
  }
  ...
  Widget _buildImageItem(int index) {
    final imagePath = cubit.selectedGalleryPaths.isNotEmpty
        ? cubit.selectedGalleryPaths[index]
        : cubit.galleryImages[index];

    final isLocalImage = cubit.selectedGalleryPaths.contains(imagePath);
          ...
          child: ClipRRect(
            ...
            child: isLocalImage
                ...
                : FutureBuilder<String>(
                    future: cubit.getPresignedUrl(imagePath),
  ...
  Widget _buildBottomActions(bool loading, int imagesCount) {
    final canAddMoreImages = imagesCount < 10;
    ...
    return Column(
      children: [
        ...
        const SizedBox(height: 16),
        ElevatedButton(
          onPressed: (loading || !canAddMoreImages) ? null : onPickImages,
          child: Text(
            imagesCount > 0 ? "Добавить еще изображения" : "Добавить изображения",
          ),
        ),
        if (!canAddMoreImages) ...[
