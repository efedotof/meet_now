...
public interface UserRepository extends JpaRepository<User, UUID>, JpaSpecificationExecutor<User> {
        ...
        @Query(value = """
                            ...
                            WHERE u.id <> :currentUserId
                              ...
                              AND u.is_searchable = true
                              AND u.is_card_mode = true
                              AND NOT EXISTS (
                                  SELECT 1 FROM swipes s
                                  WHERE s.swiper_id = :currentUserId AND s.target_id = u.id
                              )
                              AND NOT EXISTS (
                                  SELECT 1 FROM matches m
                                  WHERE (m.user1_id = :currentUserId AND m.user2_id = u.id)
                                     OR (m.user1_id = u.id AND m.user2_id = :currentUserId)
                              )
                              AND (:floor IS NULL OR u.floor = :floor)
                              AND (:verified IS NULL OR u.verified = :verified)
                              AND (:onlineOnly = false OR u.is_online = true)
                            ...
                            LIMIT 1
                        """, nativeQuery = true)
        Optional<User> findRandomCardModeCandidateWithFilters(
                        @Param("currentUserId") UUID currentUserId,
                        @Param("floor") String floor,
                        @Param("verified") Boolean verified,
        ...
        @Query("SELECT u.gamePoints FROM User u")
        List<Integer> findAllGamePoints();

        List<User> findByPremiumExpiresAtBeforeAndRoles_RoleName(LocalDateTime date, String roleName);

        @Query(value = """
                        SELECT * FROM users u
                        WHERE u.id != :currentUserId
                          AND u.floor = :targetFloor
                          AND u.is_blocked = false
                          AND u.public_key IS NOT NULL
                          AND u.public_key <> ''
                          AND NOT EXISTS (
                              SELECT 1 FROM chats c
                              WHERE (c.user_id1 = u.id AND c.user_id2 = :currentUserId)
                                 OR (c.user_id2 = u.id AND c.user_id1 = :currentUserId)
                          )
                        ORDER BY RANDOM()
                        LIMIT 1
                        """, nativeQuery = true)
        Optional<User> findRandomForQuickMatch(@Param("currentUserId") UUID currentUserId,
                        @Param("targetFloor") String targetFloor);
        ...
        @Query("""
                            ...
                            WHERE u.lastLogin < :threshold
                              AND NOT EXISTS (
                                  ...
                                  WHERE r.roleName IN ('MODERATION', 'ADMIN')
                              )
                              AND u.premiumExpiresAt <= :now
                        """)
        List<User> findInactiveNonPrivilegedUsers(
