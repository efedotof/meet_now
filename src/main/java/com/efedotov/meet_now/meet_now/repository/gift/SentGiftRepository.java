...
public interface SentGiftRepository extends JpaRepository<SentGift, UUID> {
    ...
    @Query("SELECT sg.gift.id, COUNT(sg) FROM SentGift sg WHERE sg.sender.id = :userId GROUP BY sg.gift.id ORDER BY COUNT(sg) DESC")
    List<Object[]> findPopularSentGiftsByUser(@Param("userId") UUID userId);

    @Query("SELECT sg.gift.id, COUNT(sg) FROM SentGift sg WHERE sg.recipient.id = :userId GROUP BY sg.gift.id ORDER BY COUNT(sg) DESC")
    List<Object[]> findPopularReceivedGiftsByUser(@Param("userId") UUID userId);
}